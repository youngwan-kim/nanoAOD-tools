...
#	self.addObject(ROOT.TH1F('LHEpart_pt','LHEpart_pt',200,0,1000))
	...
	# rawobjects are pt-unsorted collections of objects
        rawelectrons = Collection(event, "Electron") ; rawmuons = Collection(event, "Muon") ; rawjets = Collection(event, "Jet") ; rawphotons = Collection(event, "Photon") ;
	...
	for rawelectron in rawelectrons :
	  LT += rawelectron.p4()
	  if IsCustomElectronID("MonojetLoose",rawelectron) is True :
	...
	for rawphoton in rawjets :
	  photons.append(rawphoton)
	  photons.sort(key=lambda x : x.p4().Pt())

	# Double Electron CR Selection
	#
	# 2 loose electrons
	if len(electrons) is not 2 : return False
	self.Cutflow.Fill(1)
	# at least one is tight
	for loose_electrons in electrons :
	  if IsCustromElectronID("MonojetTight",loose_electrons) is True : 
	    self.Cutflow.Fill(2)
	  else :  return False
	# pair mass in [60,120]
	pair = electrons(0).p4() + electrons(1).p4()
	if pair.M() <= 60 or pair.M() >= 120 : return False
	self.Cutflow.Fill(3) 
	# opposite charge 
	if electrons(0).__getattr__("charge")*electrons(1).__getattr__("charge") > 0 : return False
	self.Cutflow.Fill(4)
	...
	for i in range(0,min(len(electrons),4)) :
        ...
        for i in range(0,min(len(photons),4)) :
