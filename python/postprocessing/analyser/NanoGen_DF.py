...
class ExampleAnalysis(Module):
	...
	for i in range(0,2) :
		...
		if ob == 'eta' :
			...
			minval = -3.0
			maxval = 3.0
	        histlist.append(ROOT.TH1F('Lepton'+str(i)+'_'+ob,'Lepton'+str(i)+'_'+ob,binning,minval,maxval))
	...
	self.Cutflow.Fill(1)
	if len(GenLeptons) is not 2 : return False
	self.Cutflow.Fill(2)
	if GenLeptons[0].pdgId*GenLeptons[1].pdgId != 143 : return False
	self.Cutflow.Fill(3)
...
#	self.Cutflow.Fill(5)
	...
	h_mupt = [self.Lepton0_pt,self.Lepton1_pt] 
	h_mueta = [self.Lepton0_eta,self.Lepton1_eta] 
	
	h_jetpt = [self.Jet0_pt,self.Jet1_pt] 
	h_jeteta = [self.Jet0_eta,self.Jet1_eta]

	for i in range(0,2) :
	    h_mupt[i].Fill(GenLeptons[i].pt,w)
	    h_mueta[i].Fill(GenLeptons[i].eta,w)
	...
	self.DeltaPhi_ll.Fill(abs(deltaPhi(GenLeptons[0],GenLeptons[1])),w)			

	self.M_jj.Fill((GenJets[0].p4()+GenJets[1].p4()).M(),w)	  
	self.DeltaEta_jj.Fill(GenJets[0].eta-GenJets[1].eta,w)

	self.HToverLeadMuPt.Fill(HT/GenLeptons[0].pt,w)
	self.XToverLeadMuPt.Fill(XT/GenLeptons[0].pt,w)
